...
const MyOrders = () => {
    ...
    useEffect(() => {
        fetch(`https://eerie-eyeballs-55193.herokuapp.com/myorders/${user.uid}`)
            .then(res => res.json())
    ...
    const handleDelete = id => {
        const url = `https://eerie-eyeballs-55193.herokuapp.com/order/${id}`;
        fetch(url, {
            method: 'DELETE'
        })
            .then(res => res.json())
    ...
    return (
        <div>
            <h2 className="text-center my-5 green-color fs-1">All my orders</h2>
            {
